...
// 최근 프로젝트 목록을 날짜별로 그룹화하여 렌더링 (경로 단순화 포함)
function renderRecentProjectsByDateGroup(recentProjects) {
    ...
    recentProjects.forEach(project => {
        const modified = new Date(project.modified || project.lastOpened || project.date || project.timestamp || project.time || project.lastAccessed || project.lastModified || project.created);
    ...
    Object.entries(groups).forEach(([label, items]) => {
        ...
        html += items.map(project => {
            ...
            return `
                <div class="recent-project-item" title="${tooltip}">
                    <span class="recent-project-folder">${lastFolder}</span>
